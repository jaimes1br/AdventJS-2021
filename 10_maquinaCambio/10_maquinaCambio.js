 ...
 * coins[5] = 50 céntimos
 * 
 * Tenemos que crear una función que recibe el número de céntimos que hay que devolver al 
 * cliente y la función nos da un array con la combinación de monedas mínimas que debemos usar para conseguirlo.
 * 
...
/**
 * 
 * @param {Number} change Representa el cambio que se tiene que entregar
 * @returns {Array} Con el número de monedas que se deben entregar dependiendo la denominación
 */
...
function getCoins(change) {
    ...
    nCoins.forEach(coin => {
        if(temp / coin >= 1){
            let indice = coins.indexOf(coin)
            changeArray[indice] = Math.trunc(temp / coin) 
            temp = temp % coin
        }
    })
